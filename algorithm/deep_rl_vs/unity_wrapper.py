...
class UnityWrapper:
    def __init__(self,
        ...
        self._env.reset()
        # print("行为空间",self._env.behavior_specs)
        self.bahavior_name = list(self._env.behavior_specs)[0]
        print("行为空间",self.bahavior_name)
    ...
    def step(self, d_action, c_action):
        ...
        if self.d_action_size:
            # print("训练传入unity_wrapperaction", d_action)
            d_action = np.argmax(d_action, axis=1)
            # print("unity_wrapperaction",d_action)
...
if __name__ == "__main__":
    ...
    env = UnityWrapper(train_mode=True, base_port=5004)         # 创立环境
