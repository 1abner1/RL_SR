...
class Agent(object):
   ...
   def run(self, max_step):
      ...
      while not (done or step_number == max_step):
         ...
         if self.eval_mode:
            action = self.policy(torch.Tensor(obs[1][0]).to(self.device))
            ...
            if self.steps > self.expl_before:
               action = self.select_action(torch.Tensor(obs[1][0]).to(self.device))
               action = np.expand_dims(action, 0)
            else:
               # action = self.env.action_space.sample()  同sac 的问题，没有定义这个函数在unity_wrapper
               action = self.select_action(torch.Tensor(obs[1][0]).to(self.device))
               action = np.expand_dims(action, 0)
            ...
            self.replay_buffer.add(obs[1][0], action, reward, next_obs[1][0], done)
