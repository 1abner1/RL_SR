   ...
   def run(self, max_step):
      ...
      while not (done or step_number == max_step):
         ...
         if self.eval_mode:
            ...
            _, _, action, _ = self.policy(torch.Tensor(obs[1][0]).to(self.device))
            action = action.detach().cpu().numpy()
            action = np.expand_dims(action, 0)
            # c1_action =np.array([[0]])
            c1_action =None
            next_obs, reward, done, _ = self.env.step(action,c1_action)

            # Add experience to buffer
            v = self.vf(torch.Tensor(obs[1][0]).to(self.device))
            self.buffer.add(obs[1][0], action, reward, done, v)
