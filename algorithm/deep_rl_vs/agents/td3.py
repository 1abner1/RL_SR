'''
td3 算法 采用的是Twin Delayed deep deterministic policy Gradient 
是一种面向连续动作空间基于actor-critic 架构的深度强化学习算法，同时对策略网络和值网络进行改进
引入了两个Q 网络，也就是值网络和一个critic 网络
'''
...
class Agent(object):
   ...
   def train_model(self):
      ...
      q1 = self.qf1(obs1, acts).squeeze(1)   #实现降维
      q2 = self.qf2(obs1, acts).squeeze(1)

      # Target policy smoothing, by adding clipped noise to target actions
      pi_target = self.policy_target(obs2)
      epsilon = torch.normal(mean=0, std=self.target_noise, size=pi_target.size()).to(self.device)   # 正态分布
      ...
      # Save losses
      self.policy_losses.append(policy_loss.item())  #表示遍历所有自定的数值对，然后返回一个键值对列表
      ...
      while not (done or step_number == max_step):
         ...
         if self.eval_mode:
            ...
            if self.steps > self.expl_before:
               ...
               action = np.random.randint(-1,1,size=5)
               action = np.expand_dims(action, 0)
               # action = self.select_action(torch.Tensor(obs).to(self.device))
               # action = np.expand_dims(action, 0)
