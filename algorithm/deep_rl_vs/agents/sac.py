...
class Agent(object):
   ...
   def run(self, max_step):
      ...
      while not (done or step_number == max_step):
         ...
         if self.eval_mode:
            action, _, _ = self.policy(torch.Tensor(obs[1][0]).to(self.device))
            ...
            if self.steps > self.expl_before:
               _, action, _ = self.policy(torch.Tensor(obs[1][0]).to(self.device))
               action = action.detach().cpu().numpy()
               action = np.expand_dims(action, 0)
            else:
               # action = self.env.action_space.sample()    #这条代码有问题 下面三条是我自己写的
               action, _, _ = self.policy(torch.Tensor(obs[1][0]).to(self.device))
               action = action.detach().cpu().numpy()
               action = np.expand_dims(action, 0)
