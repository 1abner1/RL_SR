...
from helper.models import GRUActorCritic, SNAILActorCritic    # 使其具有记忆功能记忆住之前的动作的功能值
...
def meta_train(device, num_workers, model_type, metalearn_epochs, task, num_actions, num_states, num_tasks, num_traj, traj_len, ppo_epochs, mini_batchsize, batchsize, gamma, 
  ...
  # Create the model
  if (model_type == 'gru'):
    model = GRUActorCritic(num_actions, num_feature)   #lstm logstic door control
    # print("是否使用了GRU")
    ...
    # two difference model to create model
  ...
  for i in range(metalearn_epochs):
    ...
    # take agent to meta_learn algorthrim
    # print("3333333333333333333333333333333")
    # core train frame to agent  就是ppo 的网络结构结构，进行实例化处理了
    meta_learner.train(agent)    #将ppo 策略传给元强化学习器。
    # print("把ppo策略传给元强化学习器")
  ...
  meta_learner.sampler.envs.close()   #关闭采样的任务器
  return model
