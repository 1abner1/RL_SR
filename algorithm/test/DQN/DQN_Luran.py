...
class Agent():
    def __init__(self,state_size,action_size,seed):
        ...
        self.t_step = 0
    def step(self,state,action,reward,next_state,done,episode,writer):
        self.memory.add(state,action,reward,next_state,done)
        self.t_step = (self.t_step+1)%UPDATE_EVERY
        if self.t_step == 0:
            if len(self.memory)>BATCH_SIZE:
                experiences = self.memory.sample()
                self.learn(experiences,GAMMA,episode,writer)
    ...
    def learn(self,experiences,gamma,episode,writer):
...
if __name__ == '__main__':
    ...
    score = []
    max_episode = 200000000000000000000000000000000000000000
    max_t = 1000
    eps_start = 1.0
    eps_end = 0.001
    eps_decay = 0.995
